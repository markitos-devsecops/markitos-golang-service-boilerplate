...
func TestCanCreateValidRandomEmail(t *testing.T) {
	emailRegex := `^[a-zA-Z]+@[a-zA-Z]+\.[a-z]{2,4}$`
...
func TestCanCreateValidPersonalName(t *testing.T) {
	...
	require.True(t, len(name) >= validLengths[0] && len(name) <= validLengths[1])
	require.True(t, len(words) >= 1 && len(words) <= 6)
	for _, word := range words {
		word = strings.ReplaceAll(word, " ", "")
		require.True(t, regexp.MustCompile(`^[a-z]+$`).MatchString(word))
	}
}

func TestCanCreateValidSlug(t *testing.T) {
	validLengths := []int{3, 75}
	slug := domain.Slug()
	words := strings.Split(slug, "-")

	require.NotEmpty(t, slug)
	require.True(t, len(slug) >= validLengths[0] && len(slug) <= validLengths[1])
	require.True(t, len(words) >= 1 && len(words) <= 6)
	for _, word := range words {
		word = strings.ReplaceAll(word, " ", "")
		require.True(t, regexp.MustCompile(`^[a-z]+$`).MatchString(word))
